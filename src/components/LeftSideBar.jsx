  ...
  return (
    ...
    >
      <nav className="p-4 space-y-2 max-h-[calc(100%-40px)] overflow-auto">
          ...
          {menuItems.length > 0 &&
            menuItems.map(({ label, link, icon, active }) => (
              <li key={label}>
                <Link
                  to={link}
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg dark:hover:bg-gray-800 ${active ? "bg-blue-50 dark:bg-gray-950 dark:text-orange-300 text-blue-600" : "text-gray-700 dark:text-gray-300 hover:bg-gray-100"} font-medium`}
                >
                  <span>{icon}</span>
                  {label}
                </Link>
        ...
        <div className="fixed flex bottom-0 w-full border-t border-t-gray-400 dark:border-t-gray-800 -ml-4 dark:bg-gray-900 bg-white">
          <button className="flex items-center gap-3 px-4 py-4 rounded-lg dark:text-orange-300 hover:bg-red-50 text-red-600">
            <span>🚪</span>
            Logout
          </button>
        </div>
